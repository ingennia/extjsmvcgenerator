Ext.define('<%= app_name %>.controller.<%= model_name %>', {
	...
	init: function(){
		this.control({
			...
			'map<%= model_name %> > toolbar #select': {
				click: this.selectfeature
			},

			'map<%= model_name %> > toolbar #new': {
				click: this.newfeature
			},

			'map<%= model_name %> > toolbar #rotate': {
				click:  this.rotatefeature
			},
			'map<%= model_name %> > toolbar #move': {
				click:  this.movefeature
			},

			'map<%= model_name %> > toolbar #delete': {
				click:  this.deletefeature
			},

			'map<%= model_name %> > toolbar #layerCombo': {
				select:  this.layerselect
			},

			'map<%= model_name %> > toolbar #logout': {
				click:  this.logout
			},
